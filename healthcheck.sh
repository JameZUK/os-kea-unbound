#!/bin/sh
echo "=========================================================="
echo "    os-kea-unbound Health Check (v3.8.0)"
echo "=========================================================="

# 1. Check UI/Model Patch Status (DHCPv4)
echo -n "[1/11] Checking OPNsense MVC Patches (DHCPv4)... "
...
echo -n "[2/11] Checking OPNsense MVC Patches (DHCPv6)... "
...
echo -n "[3/11] Checking Active Kea Config... "
...
# 4. Check Hook Script
echo -n "[4/11] Checking Hook Script... "
[ -x "/usr/local/share/kea/scripts/kea-unbound-hook.sh" ] && echo "OK" || echo "FAILED"

# 5. Check Plugin Registration
echo -n "[5/11] Checking Plugin Registration... "
[ -f "/usr/local/etc/inc/plugins.inc.d/keaunbound.inc" ] && echo "OK" || echo "FAILED"

# 6. Check Persistence Hooks (update + early-boot repair + start-boot replay)
echo -n "[6/11] Checking Persistence Hooks... "
UPDATE_OK=false; BOOT_OK=false; REPLAY_OK=false
...
[ -x "/usr/local/etc/rc.syshook.d/start/50-keaunbound-replay" ] && REPLAY_OK=true
if $UPDATE_OK && $BOOT_OK && $REPLAY_OK; then echo "OK (update + early + replay)"
else
    MISSING=""
    $UPDATE_OK || MISSING="$MISSING update"
    $BOOT_OK || MISSING="$MISSING early"
    $REPLAY_OK || MISSING="$MISSING replay"
    echo "PARTIAL (missing:$MISSING)"
fi

# 7. Check Log Rotation
echo -n "[7/11] Checking Log Rotation... "
[ -f "/usr/local/etc/newsyslog.conf.d/keaunbound.conf" ] && echo "OK" || echo "FAILED"

# 8. Check Unbound Connectivity
echo -n "[8/11] Checking Unbound Control... "
...
echo -n "[9/11] Checking Python3 (IPv6 PTR)... "
...
echo -n "[10/11] Checking drill (DNS lookup)... "
if command -v drill >/dev/null 2>&1; then echo "OK"
else echo "FAILED (drill required for dual-stack record preservation)"; fi

# 11. Check status helper
echo -n "[11/11] Checking keaunbound-status helper... "
[ -x "/usr/local/bin/keaunbound-status" ] && echo "OK" || echo "FAILED"
